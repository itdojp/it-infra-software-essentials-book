...
```mermaid
graph TD
    subgraph "データ記述言語の全体像と応用"
        subgraph "主要なデータ記述言語"
            JSON["JSON<br/>(JavaScript Object Notation)"]
            YAML["YAML<br/>(YAML Ain't Markup Language)"]
            XML["XML<br/>(eXtensible Markup Language)"]
            TOML["TOML<br/>(Tom's Obvious Minimal Language)"]
            
            JSON_Features["特徴:<br/>・シンプルで軽量<br/>・Web標準<br/>・機械読み取り重視"]
            YAML_Features["特徴:<br/>・人間が読みやすい<br/>・インデントベース<br/>・設定ファイルに最適"]
            XML_Features["特徴:<br/>・厳密な構造<br/>・名前空間対応<br/>・レガシーシステム"]
            TOML_Features["特徴:<br/>・設定ファイル特化<br/>・シンプルな構文<br/>・型安全"]
        end
        
        subgraph "JSON vs YAML比較"
            Comparison["構文比較例"]
            JSONEx["JSON:<br/>{<br/>  \"server\": {<br/>    \"name\": \"web-01\",<br/>    \"port\": 8080,<br/>    \"ssl\": true<br/>  }<br/>}"]
            YAMLEx["YAML:<br/>server:<br/>  name: web-01<br/>  port: 8080<br/>  ssl: true"]
            
            JSONPros["JSON利点:<br/>・軽量で高速パース<br/>・API通信標準<br/>・厳密な構文"]
            YAMLPros["YAML利点:<br/>・可読性が高い<br/>・コメント記述可能<br/>・複雑な構造表現"]
        end
        
        subgraph "インフラでの活用場面"
            API["API通信<br/>REST/GraphQL"]
            Config["設定ファイル<br/>アプリケーション設定"]
            IaC["Infrastructure as Code<br/>Terraform/Ansible"]
            Container["コンテナ<br/>Docker/Kubernetes"]
            CI_CD["CI/CD<br/>GitHub Actions/GitLab CI"]
            
            API_Detail["・リクエスト/レスポンス<br/>・認証トークン<br/>・エラー情報"]
            Config_Detail["・データベース接続<br/>・環境変数<br/>・機能フラグ"]
            IaC_Detail["・リソース定義<br/>・プレイブック<br/>・状態管理"]
            Container_Detail["・Dockerfile<br/>・compose.yml<br/>・マニフェスト"]
            CI_Detail["・ワークフロー定義<br/>・ビルド設定<br/>・デプロイ手順"]
        end
        
        subgraph "Pythonでの操作"
            JSONLib["jsonライブラリ<br/>標準搭載"]
            YAMLLib["PyYAMLライブラリ<br/>pip install PyYAML"]
            
            JSONOps["操作:<br/>・json.loads() / json.dumps()<br/>・json.load() / json.dump()<br/>・辞書⇔JSON変換"]
            YAMLOps["操作:<br/>・yaml.safe_load() / yaml.dump()<br/>・設定ファイル読み込み<br/>・構造化データ操作"]
        end
    ...
    style IaC fill:#ffe0b2
    style JSONLib fill:#e1f5fe
```
