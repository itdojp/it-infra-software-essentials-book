...
- API の役割と、インフラ運用で API 操作が必要になる理由を説明できる。
- RESTful API の基本概念（リソース、URI、HTTP メソッド、ステータスコード）を整理できる。
- Python（requests）を使って API を呼び出し、レスポンスを扱う流れを理解できる。
- 認証・認可を含む API 連携で、最低限押さえるべき観点を把握できる。
...
### **HTTP メソッドの基本（GET, POST, PUT, PATCH, DELETE）**
...
- **PATCH**
  - **意味**: リソースへの部分的な変更（Update）。
  - **利用目的**: `patch document`（変更指示を表す文書）を適用してリソースを変更します。リソース全体を置き換える PUT とは異なります。
  - **例**: サーバー設定の一項目だけを変更する、ユーザープロフィールの一部だけを更新する。
  - **冪等性**: PATCH 自体は安全（safe）でも冪等（idempotent）でもありません。patch document の形式と処理によって個別のリクエストを冪等に設計できる場合はありますが、再試行可能と決めつけず API 仕様を確認します。基準バージョンに依存する変更では、`If-Match` と strong ETag などの条件付きリクエストを使用します。
- **DELETE**
  - **意味**: リソースの削除（Delete）。
...
PATCH と PUT の処理差、patch document、冪等性、条件付きリクエストの定義は、[RFC 5789 Section 2](https://www.rfc-editor.org/rfc/rfc5789#section-2)を参照してください。
...
def get_oauth_token() -> str:
    ...
    if not client_id or not client_secret:
        raise ValueError("OAUTH_CLIENT_ID / OAUTH_CLIENT_SECRET が設定されていません")
    ...
    raise RuntimeError(f"OAuth 認証エラー: {response.status_code} - {response.text}")


def make_authenticated_request(endpoint: str, method: str = "GET", data=None) -> requests.Response:
...
- API 呼び出しはタイムアウト、再試行、レート制限（429）、冪等性を前提に設計します（安全に再実行できる手順にする）。
- 認証情報（API キー/トークン）はハードコードせず、環境変数やシークレット管理へ退避します。ログ出力時はマスキングします。
- 書き込み系 API は request ID / idempotency key の有無を確認し、再実行時に重複作成や意図しない削除が起きない設計にします。
- `curl` や GUI クライアントの履歴にトークンや Cookie が残らないよう、検証後に履歴、環境変数、共有ワークスペースを確認します。
- API 仕様は変更されます。利用前に対象バージョンの公式ドキュメントで要確認とし、互換性破壊に備えて検証手順を残します。

## まとめ

- API は「機能やデータを利用するための窓口」であり、クラウド運用では多くの操作が API として提供される。
- RESTful API は HTTP メソッドで操作を表現し、URI でリソースを識別する設計が基本になる。
- ステータスコードやレスポンス形式を踏まえて、エラー時の扱い（再試行や例外処理）を設計することが重要になる。
- Python（requests）を使うと、クラウドや SaaS の API を呼び出す自動化を組み込みやすい。

次章では、Git・正規表現・データ構造など、インフラ運用を支える周辺のソフトウェア知識を扱います。

## 次に読む

- [第5章：インフラ運用を支えるその他のソフトウェア知識](../chapter05/)
- [目次に戻る](../../)
