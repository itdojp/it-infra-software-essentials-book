...
* APIの基本概念と、HTTPメソッド/ステータスコードをインフラ運用の文脈で説明できるようになる。  
* RESTful APIの考え方を理解し、リソース操作（取得/作成/更新/削除）のイメージを持てるようになる。  
* Python（requests）でAPIを呼び出し、レスポンス処理・エラー処理の基本形を作れるようになる。  
...
* クラウド運用の多くは「API操作」として実装でき、スクリプト自動化の拡張先になります。  
* RESTful APIでは、HTTPメソッドとステータスコードを手がかりに「何が起きたか」を判断できます。  
* requests を使う場合は、タイムアウト・例外処理・認証情報の扱い（環境変数など）をセットで考えると、実務で崩れにくい実装になります。  

## **次章へのつなぎ**

第5章では、運用を支える周辺スキルとして Git・正規表現・データ構造を扱います。API連携や自動化の成果物（設定ファイル/スクリプト）をチームで安全に運用するための基礎として活用してください。  
