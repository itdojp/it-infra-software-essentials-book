...
* インフラエンジニアにソフトウェア知識が必要な理由（自動化、IaC、クラウドネイティブ、DevOps/SRE など）を説明できるようになる。  
* 本書の対象読者・到達目標・学習の進め方を把握し、自分の読み方（通読/目的別）を決められるようになる。  
* 5章構成の位置づけを理解し、どの順序で学ぶと実務に接続しやすいかをイメージできるようになる。  
...
* 現代のインフラ運用では、自動化・IaC・クラウド連携の前提としてソフトウェア知識が必要です。  
* 本書は、設定ファイル（JSON/YAML）、スクリプト（シェル/Python）、API連携、Git/正規表現/データ構造を「実務の入口として使える」レベルまで短時間で押さえる構成です。  
* 章の順序は、データ形式 → 自動化 → API連携 → 周辺知識（Git等）という学習曲線に沿っています。  

## **次章へのつなぎ**

第2章では、設定ファイルやAPI通信で頻出する JSON と YAML を扱います。まずは「構造を読める/書ける」状態を作り、以降のスクリプト・API連携の土台にします。  
