...
* JSONの基本構造（オブジェクト/配列/データ型）を理解し、Pythonで読み書きできるようになる。  
* YAMLの基本構造（マッピング/シーケンス）とインデントのルールを理解し、設定ファイルとして扱えるようになる。  
* JSON/YAML/その他形式（XML, TOML/INI, CSV など）の使い分けの観点を持てるようになる。  
...
* JSON と YAML は、インフラの設定・自動化・API連携で頻出する「基本のデータ形式」です。  
* Pythonを使うと、設定ファイルやAPIレスポンスを「読み込む → 検証する → 変換する → 書き戻す」という処理を実装できます。  
* XML/TOML/CSV などの形式は、目的に応じて登場します。まずは存在と得意分野を押さえ、必要になったときに深掘りする前提で問題ありません。  

## **次章へのつなぎ**

第3章では、シェルスクリプトとPythonを使って、日々の運用タスクを自動化する基本形を学びます。第2章で扱った JSON/YAML を「スクリプトで扱う」場面も増えるため、構造の読み方をここで固めておくと進めやすくなります。  
