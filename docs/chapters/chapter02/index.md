...
- JSON の基本構造（オブジェクト、配列、データ型）と記述ルールを説明できる。
- YAML の基本構造と記述ルール（インデント、リスト、辞書など）を説明できる。
...
JSONは、軽量なデータ交換フォーマットとして広く利用されています。JavaScriptのオブジェクト表記法をベースにしており、人間が読みやすく、機械が解析しやすいという特徴を持っています。Web APIのレスポンスやリクエストボディ、各種設定ファイルなどで頻繁に利用されます。
...
上記のコードをファイルに保存して実行すると、標準出力に結果が表示され、カレントディレクトリに `config.json` と `users.json` が作成されます。

```bash
# 例: json_example.py として保存した場合
...
'config.json' を作成しました。
'config.json' から読み込んだデータ: {'database': 'mydb', 'port': 5432, 'environment': 'production'}
'users.json' にデータを書き込みました。
```
...
辞書やリストの操作に慣れていれば、JSONデータの操作も直感的に行えます。
...
- JSON は仕様が厳密です（コメント不可、末尾カンマ不可など）。保存・配布前にパーサで機械検証します（例: `python -m json.tool`、`jq`）。
- YAML は表現力が高い一方で、インデントや暗黙の型変換により意図しない差分が出やすい形式です。疑わしい値はクォートし、利用するパーサ（ツール）の挙動で要確認とします。
- 設定ファイルに秘密情報（API キー、トークン、パスワード等）を直接埋め込まない運用にします（環境変数/シークレット管理への退避）。
- サンプルコードは学習目的です。実務では入力検証、例外処理、権限（ファイルパーミッション）を要件に合わせて追加します。

## まとめ

- JSON は API 通信や設定表現で広く使われ、構造（オブジェクト/配列）と型を理解すると読み書きが安定する。
- YAML は人間に読みやすい記述ができる一方、インデントや表記のばらつき（同じ内容でも複数の書き方が存在すること）で意図せぬ差分が出やすいので、基本ルールの理解が重要になる。
- Python の標準/周辺ライブラリを使うと、JSON/YAML を「読み込む → 加工する → 書き出す」を自動化できる。
- XML/TOML/CSV なども含め、目的（機械処理のしやすさ、可読性、互換性）に応じて形式を選ぶ。

次章では、シェルスクリプトと Python を使って、定型作業を自動化するための基礎を扱います。

## 次に読む

- [第3章：スクリプト言語による自動化の初歩](../chapter03/)
- [目次に戻る](../../)
