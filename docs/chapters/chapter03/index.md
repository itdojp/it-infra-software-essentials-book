...
* シェルスクリプトの基本要素（変数、条件分岐、ループ、パイプ/リダイレクト、関数など）を理解し、簡単な自動化を組み立てられるようになる。  
* Pythonを使った運用自動化の入口（ファイル操作、外部コマンド実行、例外処理など）を押さえ、段階的にスクリプトを強化できるようになる。  
* 自動化で頻出する処理（ログ解析、設定ファイル処理、ヘルスチェック等）を「実装の型」としてイメージできるようになる。  
...
* シェルスクリプトは、OS標準コマンドを組み合わせて「すぐ効く自動化」を作るのに向いています。  
* Pythonは、例外処理やデータ構造の扱い、外部API/SDK連携などを含む「拡張しやすい自動化」に向いています。  
* 自動化は、まず小さく作って動かし、検証・ログ・例外処理を足して堅牢化する進め方が現実的です。  

## **次章へのつなぎ**

第4章では、APIを通じてクラウドサービスやSaaSと連携する方法を学びます。第3章で押さえた「スクリプトの基本形」を、requests等のHTTPクライアントに接続するイメージで読み進めると理解しやすくなります。  
